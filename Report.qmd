...
These concerns bring about our motivating question: is there empirical evidence of an association between the reforms and the rate at which the medical residents passed the exam?
...
To check goodness of fit, we plotted Pearson residuals over time. We appear to minimize residuals over time in the mixture model, as seen in Figure 2b. On the other hand, for both the full and subset beta-binomial models, there is no apparent patterns for the residuals in figures 2a and 2b. Putting this together, we concluded that the binomial mixture model is the best fit for the data (*see: Figure 2*).
...
combined <- p1 + p2 + 
  plot_annotation(
    caption = str_wrap("Figure 2a and 2b: Pearson Residuals Plotted by Year for Full Beta-Binomial Model Compared to Subsetted Model Fits and Mixture Binomial Model"),
...
ggplot(data, aes(x = Year, y = Pass/N)) +
  ...
  labs(title = "Pass Rates by Year with GLMM (binomial) Step Fit",
       y = "Pass Rate",
       caption = str_wrap("Figure 3: Internal medicine examination pass rates are plotted against year. Horizontal solid lines represent pass rates chosen by the binomial mixture model for no reform, reform one, and reform two. Dotted lines represent 95% confidence interval for pass rate.")
       ) + 
